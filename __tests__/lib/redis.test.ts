...
describe("redis error logging with logRedisError", () => {
  ...
  describe("ZADD operations", () => {
    it("logs error when redis.zadd fails", async () => {
      ...
      const result = await (redis.zadd as unknown as (key: string, score: number, member: string, score2?: number, member2?: string) => Promise<number>)("zset_key", 1, "member1", 2, "member2");
    ...
    it("returns 0 when redis.zadd fails", async () => {
      ...
      const result = await (redis.zadd as unknown as (key: string, score: number, member: string) => Promise<number>)("key", 1, "member");
  ...
  describe("HSET operations", () => {
    it("logs error when redis.hset fails", async () => {
      ...
      const result = await (redis.hset as unknown as (key: string, field: string, value: string) => Promise<number>)("hash_key", "field", "value");
    ...
    it("returns 0 when redis.hset fails", async () => {
      ...
      const result = await (redis.hset as unknown as (key: string, field: string, value: string) => Promise<number>)("key", "field", "value");
