...
describe("transferBonuses.ts internal integrity", () => {
  it("has no duplicate transfer pairs", () => {
    const counts = new Map<string, number>();
    for (const transfer of TRANSFER_BONUSES) {
    ...
    if (duplicates.length > 0) {
      throw new Error(
        `transferBonuses.ts contains duplicate transfer pairs:\n` +
        duplicates.map((entry) => `  • ${entry}`).join("\n") +
        "\n\nMerge duplicate transfer rows so transfer pricing remains deterministic.",
      );
    }
  });
});
