...
export function DealSpotlight({ lang, onDealClick }: Props) {
  ...
  return (
    <button
      ...
      className="w-full text-left group relative overflow-hidden rounded-2xl border border-amber-500/30 bg-gradient-to-r from-amber-950/40 to-surface hover:from-amber-900/50 transition-all duration-200"
    >
      {/* Glow accent */}
      ...
      <div className="relative px-4 py-3.5 flex items-center gap-4">
        ...
        <div className="flex-shrink-0 flex flex-col items-center gap-1">
          ...
          <span className="text-[9px] font-bold text-amber-400/70 uppercase tracking-widest writing-vertical hidden sm:block" style={{ writingMode: "vertical-rl", transform: "rotate(180deg)" }}>
            {t.label}
          </span>
        </div>
        ...
        <div className="flex-shrink-0 text-right">
          ...
          <div className="font-black text-amber-400 text-lg leading-tight">
            {deal.milesRequired.toLocaleString()}
            <span className="text-xs font-normal text-muted ml-0.5">pts</span>
          </div>
          {savingPct > 0 && (
            <div className="text-[10px] text-emerald-400 font-semibold">-{savingPct}%</div>
          )}
        </div>

        {/* Arrow */}
        <div className="flex-shrink-0 text-amber-400/50 group-hover:text-amber-400 group-hover:translate-x-1 transition-all text-lg">→</div>
      </div>

      {/* Bottom label on mobile */}
      <div className="sm:hidden px-4 pb-2 text-[10px] text-amber-400/60 font-semibold uppercase tracking-widest">
        {t.label}
      </div>
    </button>
  );
